...
############################################################################## 
    ...
    def __init__(self, children, parent, name = None):
        ...
        # if children == []:
        #     if name is None:
        #         raise TreeStructureError('''Tree with no children has to have 
        #         a name.''')
        ...
        #     raise TreeStructureError('''Parent type has to be same as self 
        #     type or None.''')
...
############################################################################## 
    ...
    def _link(self, other): 
        ...
        Input:      makes self parent of other and adds other to list of 
                    children of self
        """
        if self.__class__._check_height():
            if self.height <= other.height:
                raise TreeStructureError('''Child's height has to be greater  
                than parent's.''')
    ...
    def _is_type_tree(self, other):
        """
        Acts on:    a CombinatorialTree
        Input:      other, a CombinatorialTree
        Output:     nothing if the input is a CombinatorialTree, an error 
                    message if it is something else
        """
        if type(other) is not type(self):
            raise TypeError('''Input has to be a tree.''')
    ...
    def copy(self):
        ...
        """
        if self.__class__._check_height():
            newtree = MetricTree([], self.parent, self.height, self.name)
        else:
            newtree = CombinatorialTree([], self.parent, self.name)
        for child in self.children:
    ...
    def __str__(self):
        ...
        """
        return pexp.Pexp.FromTree(self).string
    ...
    def FromPexp(cls, pexp_input):
        ...
        if pexp_input.is_leaf:
            ...
            if cls._check_height():
                ...
                return cls(children_trees, None)
...
############################################################################## 
    ...
    def __init__(self, children, parent, height, name = None):
        CombinatorialTree.__init__(self, children, parent, name)
        self.__height = height
        
        for child in children:
            if child.__height >= self.__height:
                raise TreeStructureError('''Height of children has to be 
                smaller than height of parent.''')
                
        
        if type(height) is not float and type(height) is not np.float64:
            raise TypeError('''Second input (height) has to be a float.''')
...
##############################################################################
    ...
    def is_leaf(self):
        ...
        Type:       property
        """
        # return self.height == 0 and self.children == []
        return self.children == []
...
##############################################################################
    ...
    def demote(self):
        ...
        return CombinatorialTree(children, self.parent, self.name)
